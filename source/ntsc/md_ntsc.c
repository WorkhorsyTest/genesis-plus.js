                   ...
                   int in_width, int vline)
{
  ...
  blit_data.md_pixel2_ = MD_NTSC_ADJ_IN( table[*input++] );
  blit_data.kernel2  = MD_NTSC_IN_FORMAT( ntsc, blit_data.md_pixel2_ );
  blit_data.md_pixel3_ = MD_NTSC_ADJ_IN( table[*input++] );
  ...
  for ( n = chunk_count; n; --n )
  {
    ...
    MD_NTSC_COLOR_IN(&blit_data, 0, ntsc, MD_NTSC_ADJ_IN( table[*input++] ) );
    MD_NTSC_RGB_OUT(&blit_data, 0);
    MD_NTSC_RGB_OUT(&blit_data, 1);

    MD_NTSC_COLOR_IN(&blit_data, 1, ntsc, MD_NTSC_ADJ_IN( table[*input++] ) );
    MD_NTSC_RGB_OUT(&blit_data, 2);
    MD_NTSC_RGB_OUT(&blit_data, 3);

    MD_NTSC_COLOR_IN(&blit_data, 2, ntsc, MD_NTSC_ADJ_IN( table[*input++] ) );
    MD_NTSC_RGB_OUT(&blit_data, 4);
    MD_NTSC_RGB_OUT(&blit_data, 5);

    MD_NTSC_COLOR_IN(&blit_data, 3, ntsc, MD_NTSC_ADJ_IN( table[*input++] ) );
