...
version(LOG_SVP) {
const int EL_SVP     = 0x00004000; /* SVP stuff  */
const int EL_ANOMALY = 0x80000000; /* some unexpected conditions (during emulation) */
...
struct ssp1601_t {
  union mem {
    u16[256*2] RAM;  /* 2 internal RAM banks */
    struct bank_t {
      u16[256] RAM0;
      u16[256] RAM1;
    }
  ...
  ssp_reg_t[16] gr;  /* general registers */
  union ptr_t {
    u8[8] r;  /* BANK pointers */
    struct bank_t {
      u8[4] r0;
      u8[4] r1;
    }
  ...
  ptr_t ptr;
  u16[6] stack;
  u32[2][6] pmac;  /* read/write modes/addrs for PM0-PM5 */
  const int SSP_PMC_HAVE_ADDR  = 0x0001; /* address written to PMAC, waiting for mode */
  const int SSP_PMC_SET        = 0x0002; /* PMAC is set */
  const int SSP_HANG           = 0x1000; /* 68000 hangs SVP */
  const int SSP_WAIT_PM0       = 0x2000; /* bit1 in PM0 */
  const int SSP_WAIT_30FE06    = 0x4000; /* ssp tight loops on 30FE08 to become non-zero */
  const int SSP_WAIT_30FE08    = 0x8000; /* same for 30FE06 */
  const int SSP_WAIT_MASK      = 0xf000;
  u32 emu_status;
  u32[30] pad;
}
...
version(LOG_SVP) {
    ...
    rPMC.b.l = d;
version(LOG_SVP) {
    elprintf(EL_SVP, "PMC w a %04x @ %04x", rPMC.b.h, GET_PPC_OFFS());
}
  }
}
